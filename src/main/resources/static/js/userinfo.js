...
//已点赞的分页
function inLikedeach(CP) {
    eachhtml = "";
    $.ajax({
        type:"GET",
        url:"/blog/getLikedBlogsPageMessage",
        data:{
        ...
        success:function (resp) {
            ...
            //eachgetData(1)是之后显示数据的方法
            eachhtml +="<li  class='"+shouyeclass+"' >" +
                "          <a class=\"page-link \"  href=\"javascript:void(0);\" onclick='inLikedeachgetData(1)' tabindex=\"-1\">首页</a>" +
                "       </li>"
            for (var i = 1 ; i <= totalPage;i++){
                if (currentPage == i){
                    eachclass = "page-item active"
                ...
                eachhtml += " <li  class='"+eachclass+"' ><a class=\"page-link\" href=\"javascript:void(0);\" onclick='inLikedeachgetData("+i+")'>"+i+"<span class=\"sr-only\">(current)</span></a></li>"
            }
            if(currentPage!=totalPage){
                eachhtml+= "<li class=\"page-item\" >" +
                    "           <a class=\"page-link\" href=\"javascript:void(0);\" onclick='inLikedeachgetData("+(currentPage+1)+")'>下一页</a>" +
                    "       </li>"
            }else{
                eachhtml +="";
            }
            $("#eachdataLiked").html(eachhtml);
        }
    })
...
//获取每页的数据
function inLikedeachgetData(CP){
    //获取点赞过的blog
    datehtml="";
    $.ajax({
        type:"GET",
        url:"/blog/showLikedBlogsCurrentPage",
        data:{
            "currentPage":CP,
        },
        success:function (resp) {
